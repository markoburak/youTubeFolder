...
def add_category():
    if request.method == "POST":
        ...
        emoji = request.form.get('category_emoji', '📁')

        if not name or name.strip() == "":
            flash('Category name cannot be empty!', category='error')
            return redirect(url_for('category_view.index'))

        item = Category(name=name, emoji=emoji, user_id=current_user.id, created_date=datetime.date.today())
        db.session.add(item)
        db.session.commit()
    return redirect(url_for('category_view.index'))
...
def update_category(category_id):
    ...
    emoji_to_update = request.form.get('category_update_emoji')
    category_to_update.name = name_to_update
    if emoji_to_update:
        category_to_update.emoji = emoji_to_update
    db.session.commit()
